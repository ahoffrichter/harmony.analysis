...
create_plate_results <- function(
  ...
  df <- unique(data_filtered_list[[paste0(
    feature_for_filter,
    "_filtered"
  )]][, c("Row", "Column", "Timepoint")])
  rownames(df) <- NULL
  n_rows <- nrow(df)
  ...
  if (!is.null(number_of_objects)) {
    matched_names <- find_partial_list_elements(
      data_filtered_list,
      number_of_objects
    )
    ...
    for (i in seq_along(matched_names)) {
      df <- df |>
        left_join(
          data_filtered_list[[matched_names[i]]] |>
            group_by(Row, Column, Timepoint) |>
            summarise(
              !!paste0(names(matched_names)[i], "_number_of_objects") := n() /
                length(unique(Field))
            )
        )
    }
  ...
  if (!is.null(Mean_per_Well)) {
    ...
    for (i in seq_along(targets_list)) {
      df <- df |>
        left_join(
          data_filtered_list[[targets_list[[i]]$list_element]] |>
            group_by(Row, Column, Timepoint) |>
            summarise(
              !!paste0(
                targets_list[[i]]$column_name,
                "_Mean_per_Well"
              ) := mean(.data[[targets_list[[i]]$column_name]])
            )
        )
    }
  }
          ...
          data_filtered_list[[targets_list[[i]]$list_element]] |>
            ...
            summarise(
              !!paste0(
                targets_list[[i]]$column_name,
                "_Sum_per_Well"
              ) := sum(.data[[targets_list[[i]]$column_name]]) /
                length(unique(Field))
            )
        )
    }
          ...
          data_filtered_list[[targets_list[[i]]$list_element]] |>
            ...
            summarise(
              !!paste0(
                targets_list[[i]]$column_name,
                "_Max_per_Well"
              ) := max(.data[[targets_list[[i]]$column_name]])
            )
        )
    }
  }
          ...
          data_filtered_list[[targets_list[[i]]$list_element]] |>
            ...
            summarise(
              !!paste0(
                targets_list[[i]]$column_name,
                "_Min_per_Well"
              ) := min(.data[[targets_list[[i]]$column_name]])
            )
        )
    }
  }
          ...
          data_filtered_list[[targets_list[[i]]$list_element]] |>
            ...
            summarise(
              !!paste0(
                targets_list[[i]]$column_name,
                "_StdDev_per_Well"
              ) := sd(.data[[targets_list[[i]]$column_name]])
            )
        )
    }
  }
          ...
          data_filtered_list[[targets_list[[i]]$list_element]] |>
            ...
            summarise(
              !!paste0(
                targets_list[[i]]$column_name,
                "_Median_per_Well"
              ) := median(.data[[targets_list[[i]]$column_name]])
            )
        )
    }
  }
  ...
  if (!is.null(spots_per_length_neurite)) {
    ...
    df <- df |>
      left_join(
        data_filtered_list[[targets_list[[1]]$list_element]] |>
          group_by(Row, Column, Timepoint) |>
          summarise(
            Neurite.Segments...Segment.Length_Sum_per_Well = sum(
              # Divide by n_fields: spots count (from number_of_objects) is per FOV,
              # so neurite length must also be per FOV for the ratio to be comparable.
    ...
    for (i in seq_along(spots_per_length_neurite)) {
      df <- df |>
        mutate(
          !!paste0(
            spots_per_length_neurite[i],
            "_per_length_neurite"
          ) := .data[[matched_names[i]]] /
            (.data[["Neurite.Segments...Segment.Length_Sum_per_Well"]] *
              chosen_objective) *
            100
        )
    }
  ...
  if (!is.null(xx_segments_per_neurons)) {
    # mean per FOV of "xx segments - neurite segment length µm" / neuronal nuclei number of objects
    # Dividing by length(unique(Field)) matches the per-FOV scale of neuronal_nuclei_number_of_objects
    # (which is also n() / n_fields), so the ratio is comparable across wells with different FOV counts.
    targets_list <- find_partial_columns(data_filtered_list, "Segment.Length")
    df <- df |>
      left_join(
        data_filtered_list[[targets_list[[1]]$list_element]] |>
    ...
    for (i in seq_along(xx_segments_per_neurons)) {
      df <- df |>
        mutate(
          !!paste0(
            xx_segments_per_neurons[i],
            "length_per_neurons"
          ) := .data[[matched_names[i]]] /
            .data[["neuronal_nuclei_number_of_objects"]]
        )
    }
  }
  ...
  if (isTRUE(Neurite_Length_per_neuron)) {
    ...
    df <- df |>
      left_join(
        data_filtered_list[[targets_list[[1]]$list_element]] |>
          ...
          summarise(
            Neurite.Segments...Segment.Length_Sum_per_Well = sum(
              # Divide by n_fields so the numerator is on the same per-FOV scale as
              # neuronal_nuclei_number_of_objects (which is n() / n_fields).
              # This ensures Neurite_Length_per_neuron is comparable across wells
              # with different numbers of surviving FOVs after filtering.
              .data[[targets_list[[1]]$column_name]]
            ) /
              length(unique(Field))
          )
    ...
    df <- df |>
      mutate(
        Neurite_Length_per_neuron = (Neurite.Segments...Segment.Length_Sum_per_Well *
          chosen_objective) /
          neuronal_nuclei_number_of_objects
      )
  }
  ...
  if (!is.null(proportion_x_y)) {
    ...
    if (!every(proportion_x_y, ~ length(.) == 2)) {
      stop(
        "proportion_x_y must be a vector of length 2, or a list of vectors of length 2"
      )
    }

    targets_list <- map(
      proportion_x_y,
      ~ find_partial_columns(data_filtered_list, .x)
    )
    tmp_df <- unique(data_filtered_list[[paste0(
      feature_for_filter,
      "_filtered"
    )]][, c("Row", "Column", "Timepoint")])
    rownames(df) <- NULL
    for (i in seq_along(targets_list)) {
      for (j in seq_along(targets_list[[i]])) {
        tmp_df <- tmp_df |>
          left_join(
            data_filtered_list[[targets_list[[i]][[j]]$list_element]] |>
              group_by(Row, Column, Timepoint) |>
              summarise(
                !!paste0(
                  targets_list[[i]][[j]]$column_name,
                  "_Sum_per_Well"
                ) := sum(.data[[targets_list[[i]][[j]]$column_name]]) /
                  length(unique(Field))
              )
          )
      }
      tmp_df <- tmp_df |>
        ...
        mutate(
          !!paste0(
            ...
            "_per_",
            proportion_x_y[[i]][2]
          ) := (.data[[names(tmp_df)[length(names(tmp_df)) - 1]]] /
            .data[[names(tmp_df)[length(names(tmp_df))]]])
        )
    }
  ...
  df <- df |>
    left_join(
      data_filtered_list[[paste0(feature_for_filter, "_filtered")]] |>
        group_by(Row, Column, Timepoint) |>
